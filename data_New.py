...
def data_loader(mode='train', mask_out=False):
    ...
    if not mask_out:
        return image_data, eval_data, similarity, variance
    if mask_out:
        return image_data, mask_data, eval_data, similarity, variance
...
class create_batch_w_mask(create_batch):
    ...
    def __init__(self, image, mask, mos, sim, var, batches, test=False):
        super(create_batch_w_mask, self).__init__(image, mos, sim, var, batches, test)
        self.mask = mask
    ...
    def __next__(self):
        ...
        self.i += 1
        if not self.test:
            return self.image[index], self.mask[index], self.mos[index], self.sim[index], self.var[index]  # Image & MOS
        else:
            return self.image[index], self.mask[index], self.sim[index], self.var[index]  # Image